...
class Query():
    ...
    def query_paginated(url: str, xheaders, data):
        print('p')
        Query.send_to_misp(data)
        while url:
            response = requests.get(url, headers = xheaders)
            r = response.json()
            Query.send_to_misp(r)
    ...
    def indicator_query(epoch, api_url, limit):
        payload = {
            'added_after': f'{epoch}',
            'length': f'{limit}',
            'match.status': 'active'
        ...
        if r.status_code == 200:
                ...
                Query.query_paginated(api_url, xheaders, data)
...
class DataManager():
     ...
     def log():
        logging.basicConfig(filename='/var/log/Firepy/log.txt')
...
def main():
    ...
    usrinp = args.query
    query_days = DataManager.epoch_fetch(usrinp)
    logging.info(f'Queried all data since {query_days}')
    try:
