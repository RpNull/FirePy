...
class Query():
    ...
    def Query_Paging(api_url,requestType, r, headers, limit, formatting, pathing):
        offset = 0
        try:
            while True:
                payload = '''{
                ...
                r = requests.get(api_url, data=payload, headers=headers)
    ...
    def Indicator_Query(query_days):
        ...
        Query.Query_Paging(api_url,requestType, r, xheaders, limit, formatting, pathing)
#        if r.status_code != 200:
#            raise Exception(r.text)
#        if r.status_code == 200:
#            data = r.json()
